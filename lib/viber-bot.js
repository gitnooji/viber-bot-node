...
const REQUIRED_CONFIGURATION_FIELDS = ["authToken", "name", "avatar", "adminUser"];
...
ViberBot.prototype.post = function(messages, optionalTrackingData) {
  if (messages == null) return Promise.resolve();
  messages = _.isArray(messages) ? messages : [messages];
  ...
  this._logger.info(`posting from ${self.adminUser}`);

  const resolveCallback = (response, message, resolve, reject) => {
    if (response.status != 0) reject(response);
    tokens.push(response.message_token);
    //self.emit(EventConsts.MESSAGE_SENT, message, self.adminUser);
    resolve();
  };
  ...
  _.each(messages, message => {
    promise = promise.then(() => new Promise((resolve, reject) =>
      self._postMessageFromAdmin(self.adminUser, message, optionalTrackingData).then(response => resolveCallback(response, message, resolve, reject))));
  });

  return promise.then(() => new Promise((resolve, reject) =>
    self._postMessageFromAdmin(self.adminUser, lastMessage, optionalTrackingData)
      .then(response => resolveCallback(response, lastMessage, resolve, reject))))
    .then(() => Promise.resolve(tokens));
};
...
ViberBot.prototype.onSeen = function(callback) {
  this.on(EventConsts.SEEN, callback);
};

ViberBot.prototype.onDelivered = function(callback) {
  this.on(EventConsts.DELIVERED, callback);
};
...
ViberBot.prototype._postMessageFromAdmin = function(fromAdminId, message, optionalTrackingData) {
  const jsonMessage = message.toJson();
  return this._client.postMessage(fromAdminId, jsonMessage.type, jsonMessage, optionalTrackingData);
};
