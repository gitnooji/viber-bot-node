...
ViberBot.prototype.postMessage = function(fromAdminId, message, optionalTrackingData) {
  if (message == null) return Promise.resolve();

  this._logger.info(`posting from ${fromAdminId}`);
  ...
  const resolveCallback = (response, message, resolve, reject) => {
    if (response.status != 0) reject(response);
    tokens.push(response.message_token);
    self.emit(EventConsts.MESSAGE_SENT, message, fromAdminId);
    resove();
  };
  ...
  return promise.then(() => new Promise((resolve, reject) =>
    self._postMessageFromClient(fromAdminId, message, optionalTrackingData)
      .then(response => resolveCallback(response, message, resolve, reject))))
    .then(() => Promise.resolve(tokens));
};
...
ViberBot.prototype._postMessageFromAdmin = function(fromAdminId, message, optionalTrackingData) {
  const jsonMessage = message.toJson();
  return this._client.postMessage(fromAdminId, jsonMessage.type, jsonMessage, optionalTrackingData, optionalKeyboard);
};
