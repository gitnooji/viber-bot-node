...
ViberClient.prototype.postMessage = function(fromAdminUserId, messageType, messageData, optionalTrackingData) {
  if (!fromAdminUserId || !messageType || !messageData) {
    throw new Error(`Invalid arguments passed to sendMessage. to, type and message are required`);
  }

  const request = {
    "from": fromAdminUserId,
    "sender": {
      "name": this._bot.name,
      "avatar": this._bot.avatar
    },
  ...
  this._logger.debug("Sending %s message to viber user '%s' with data", messageType.toUpperCase(), fromAdminUserId, messageData);
